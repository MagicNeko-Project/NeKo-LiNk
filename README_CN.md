...
- **🎭 Fake-TCP 插件化模式 (Fake-TCP Mode)**: 极致的穿透魔法！通过集成高性能插件 [Phantun](https://github.com/dndx/phantun)，将所有流量透明地伪装成合法的 TCP 流量。这一过程完全由 `nekolink-ctl` 自动化管理，无需复杂的手动配置喵。
...
- **🧦 本地 SOCKS5 代理**: 支持在客户端开启本地 SOCKS5 代理服务，强制绑定 `127.0.0.1`，实现安全的分应用代理，无需修改全局路由喵！
...
3. **nekolink-ctl**: 智能控制平面，自动读取配置、分派 Phantun 侧车插件、启动信令交换并拉起隧道喵。
4. **nekolink-tcp**: 包含增强版的 Phantun 插件集喵。
...
| `mode` | `ip` (Raw IP), `udp` (标准模式) 或 `tcp` (基于 Phantun 的 Fake-TCP 伪装)。这是**数据传输与信令交换**使用的协议喵。 | 推荐使用 `ip` 或 `tcp` 模式绕过限制 |
| `ip_protocol` | 当模式为 `ip` 时指定的协议号。 | 建议选择 143 到 252 之间的数字。 |
| `listen_port` | 当模式为 `udp` 时，指定 WireGuard 的监听端口（默认 51820）。在 `ip/tcp` 模式下该选项会被忽略喵。 | `ip/tcp` 模式下设为 `null` 即可 |
...
| `peers` | 对端信息。在 `ip/tcp` 模式下，只需填入对端的 **公网 IP** (如 `1.2.3.4`)，猫娘会直接通讯（并在 `tcp` 模式下自动启动侧车），实现 **100% 纯净隐藏，彻底告别 UDP 端口喵**！ | 例如 `{"endpoint": "1.2.3.4"}` |
