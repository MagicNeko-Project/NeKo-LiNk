...
- **🎭 Fake-TCP 伪装模式 (Fake-TCP Mode)**: 极致的穿透魔法！将所有流量伪装成合法的 TCP 流量（包含完整握手模拟与状态管理），让数据包在网络监控中看起来就像是在浏览网页喵。
...
| `mode` | `ip` (Raw IP), `udp` (标准模式) 或 `tcp` (Fake-TCP 伪装)。这是**数据传输与信令交换**使用的协议喵。 | 推荐使用 `ip` 或 `tcp` 模式绕过限制 |
...
| `peers` | 对端信息。在 `ip/tcp` 模式下，只需填入对端的 **公网 IP** (如 `1.2.3.4`)，猫娘会直接通讯，实现 **100% 纯净隐藏，彻底告别 UDP 端口喵**！ | 例如 `{"endpoint": "1.2.3.4"}` |
