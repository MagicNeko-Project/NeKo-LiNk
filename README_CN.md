...
| `peers` | 对端信息。在 `ip` 模式下，只需填入对端的 **公网 IP** (如 `1.2.3.4`)，猫娘会通过 Raw IP 直接通讯，实现 **100% 纯净隐藏，彻底告别 UDP 端口喵**！在 `udp` 模式下，填入对端的 `IP:信令端口`。 | 例如 `{"endpoint": "1.2.3.4:5678"}` |
| `signal_port` | **加密信令端口** (仅用于 `udp` 模式，默认为 5678)。在 `ip` 模式下，这个选项会被猫娘温柔地忽略，因为信令会自动复用你的 `ip_protocol` 进行传输喵！ | 哪怕是 `ip` 模式，初始握手也需要这个传统的 UDP 端口喵 |
