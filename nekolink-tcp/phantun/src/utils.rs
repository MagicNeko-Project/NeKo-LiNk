...
use nix::sys::socket::{
    ...
    CmsgIterator, ControlMessageOwned, MsgFlags, SockaddrLike, SockaddrStorage,
};
use nix::cmsg_space;
...
pub fn new_udp_reuseport(local_addr: SocketAddr) -> UdpSocket {
    ...
    // packets
    if local_addr.is_ipv4() {
        nix::sys::socket::setsockopt(udp_sock.as_raw_fd(), nix::sys::socket::sockopt::Ipv4PacketInfo, &true)
            .unwrap();
    } else {
...
pub async fn udp_recv_pktinfo(
    ...
    loop {
        ...
        let res = sock.try_io(Interest::READABLE, || {
            let mut control_message_buffer = vec![0u8; 128];
            ...
            let src_addr_storage = res.address.expect("missing source address");
            let src_addr: SocketAddr = {
                if let Some(inaddr) = src_addr_storage.as_sockaddr_in() {
                    SocketAddrV4::new(inaddr.ip().into(), inaddr.port()).into()
                } else if let Some(in6addr) = src_addr_storage.as_sockaddr_in6() {
                    SocketAddrV6::new(
                        in6addr.ip(),
                        in6addr.port(),
                        in6addr.flowinfo(),
