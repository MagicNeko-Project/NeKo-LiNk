...
public final class GoBackend implements Backend {
    ...
    public GoBackend(final Context context) {
        ...
        SharedLibraryLoader.loadSharedLibrary(context, "nekolink-android");
    ...
    private static native String nekoSignal(String serverAddr, String psk, String myPubKey);
    ...
    private void setStateInternal(final Tunnel tunnel, @Nullable final Config config, final State state)
        ...
        if (state == State.UP) {
            ...
            // NekoLink Signaling Logic
            Config effectiveConfig = config;
            if (config.getInterface().getNekoKey().isPresent()) {
                String psk = config.getInterface().getNekoKey().get();
                String myPubKey = config.getInterface().getKeyPair().getPublicKey().toBase64();
                Log.i(TAG, "NekoLink enabled. Starting signaling handshake for " + myPubKey);
                
                if (!config.getPeers().isEmpty()) {
                    Peer serverPeer = config.getPeers().get(0);
                    if (serverPeer.getEndpoint().isPresent()) {
                         String endpoint = serverPeer.getEndpoint().get().toString(); 
                         // Note: IntegEndpoint.toString() usually returns "host:port", which is what we need.

                         String jsonResult = nekoSignal(endpoint, psk, myPubKey);
                         Log.i(TAG, "NekoLink signal result: " + jsonResult);
                         
                         if (jsonResult != null && jsonResult.contains("\"success\":true")) {
                             String newEndpoint = extractJsonField(jsonResult, "endpoint");
                             String newPubKey = extractJsonField(jsonResult, "peer_pub_key");
                             String mtuStr = extractJsonField(jsonResult, "mtu");
                             
                             if (newEndpoint != null && newPubKey != null) {
                                 Log.i(TAG, "NekoLink handshake success! Target: " + newEndpoint);
                                 
                                 Config.Builder newConfigBuilder = new Config.Builder();
                                 newConfigBuilder.setInterface(config.getInterface()); 
                                 
                                 Peer.Builder newPeerBuilder = new Peer.Builder();
                                 try {
                                     newPeerBuilder.parseEndpoint(newEndpoint);
                                     newPeerBuilder.parsePublicKey(newPubKey);
                                     newPeerBuilder.parseAllowedIPs("0.0.0.0/0, ::/0"); 
                                     newPeerBuilder.addAllowedIps(serverPeer.getAllowedIps());
                                     if (serverPeer.getPersistentKeepalive().isPresent())
                                         newPeerBuilder.setPersistentKeepalive(serverPeer.getPersistentKeepalive().get());
                                     if (serverPeer.getPreSharedKey().isPresent())
                                          newPeerBuilder.setPreSharedKey(serverPeer.getPreSharedKey().get());
                                          
                                     newConfigBuilder.addPeer(newPeerBuilder.build());
                                     
                                     for (int i = 1; i < config.getPeers().size(); i++) {
                                         newConfigBuilder.addPeer(config.getPeers().get(i));
                                     }
                                     effectiveConfig = newConfigBuilder.build();
                                 } catch (Exception e) {
                                     Log.e(TAG, "Failed to apply NekoLink config", e);
                                 }
                             }
                         } else {
                             Log.w(TAG, "NekoLink signaling failed or returned error.");
                         }
                    }
                }
            }
            ...
            dnsRetry: for (int i = 0; i < DNS_RESOLUTION_RETRIES; ++i) {
                ...
                for (final Peer peer : effectiveConfig.getPeers()) {
