...
static PEER_MTU_MAP: OnceLock<tokio::sync::Mutex<HashMap<String, HashMap<String, u16>>>> = OnceLock::new();
...
async fn sync_lowest_mtu(interface: &str) -> Result<()> {
    let mut lock = PEER_MTU_MAP.get_or_init(|| tokio::sync::Mutex::new(HashMap::new())).lock().await;
    let peer_mtus = lock.entry(interface.to_string()).or_default();
    
    if peer_mtus.is_empty() { return Ok(()); }
    
    if let Some(&min_peer_mtu) = peer_mtus.values().min() {
        let current_mtu = get_interface_mtu(interface).unwrap_or(0);
        if current_mtu != min_peer_mtu && min_peer_mtu >= 1280 {
            tracing::info!("喵！正在同步 Mesh 网路最低 MTU: {} -> {} (接口: {})", current_mtu, min_peer_mtu, interface);
...
async fn main() -> Result<()> {
    ...
    let start_signaling_tasks = |global: GlobalConfig, instances: Arc<tokio::sync::RwLock<HashMap<String, InstanceHandle>>>, token: CancellationToken| {
        tokio::spawn(async move {
            let mut active_ports = std::collections::HashSet::new();
            let mut active_raw_protos = std::collections::HashSet::new();
            
            loop {
                if token.is_cancelled() { break; }

                // 1. 扫描所有活跃实例，发现需要监听的信令端口喵
                let (current_ports, current_raw_protos) = {
                    let lock = instances.read().await;
                    let mut ports = std::collections::HashSet::new();
                    let mut protos = std::collections::HashSet::new();
                    
                    // 始终监听全局预设端口（若非零）喵
                    if global.signal_port != 0 {
                        ports.insert(global.signal_port);
                    }
                    
                    for inst in lock.values() {
                        let config = &inst.state.config;
                        if config.native_wg_compat { continue; }

                        // 接口级端口
                        if let Some(p) = config.signal_port { ports.insert(p); }
                        // Peer 级端口
                    ...
                    // 如果没有任何服务端实例且没有显式端口，可以监听随机端口（此处简化，通常至少有一个全局端口）
                    if ports.is_empty() { ports.insert(0); }
                ...
                for &port in &current_ports {
                    if !active_ports.contains(&port) {
                        println!("ฅ^•ﻌ•^ฅ 全局信令中枢：正在开启端口 {} (UDP/TCP) 的管线喵！", port);
                        let i1 = Arc::clone(&instances);
                        let t1 = token.clone();
                        tokio::spawn(async move { if let Err(e) = run_global_udp_signaling_dynamic(i1, port, t1).await { eprintln!("UDP 信令异常退出 ({}): {:?}", port, e); } });

                        let i2 = Arc::clone(&instances);
                        let t2 = token.clone();
                        tokio::spawn(async move { if let Err(e) = run_global_tcp_signaling_dynamic(i2, port, t2).await { eprintln!("TCP 信令异常退出 ({}): {:?}", port, e); } });
                        
                        active_ports.insert(port);
                    }
                }

                // 3. 动态启动新增的 Raw IP 协议管线喵
                for &proto in &current_raw_protos {
                    if !active_raw_protos.contains(&proto) {
                        ...
                        active_raw_protos.insert(proto);
                    }
                }
...
async fn run_instance(state: NekoState, token: CancellationToken) -> Result<()> {
    ...
    // 如果 MTU 为 0 或开启 Mesh 模式，表示开启了自动同步模式喵
    if config.mtu == Some(0) || config.mesh_mode {
        ...
        async move {
            loop {
                ...
                for state in current_states {
                    ...
                    if !is_client && !state.config.mesh_mode { continue; }
                    ...
                    for peer in &state.config.peers {
                        ...
                        // 只有当目标端口匹配当前 Socket 监听端口时才通过此 Socket 发送喵
                        if target_port != signal_port && signal_port != 0 {
                            continue;
                        }
                        ...
                        if let Ok(decrypted) = cipher.decrypt(nonce, encrypted_part) {
                            if decrypted.len() >= 36 {
                                ...
                                if let Some(mtu) = peer_mtu {
                                    let mut lock = PEER_MTU_MAP.get_or_init(|| tokio::sync::Mutex::new(HashMap::new())).lock().await;
                                    let peer_mtus = lock.entry(state.config.interface.clone()).or_default();
                                    peer_mtus.insert(peer_pub_key.clone(), mtu);
                                    drop(lock);
                                    
                                    if state.config.mesh_mode || state.config.mtu == Some(0) {
                                        let _ = sync_lowest_mtu(&state.config.interface).await;
                                    }
                                }
                                ...
                                // 配置 Peer 喵
                                let _ = configure_peer(&state.config.interface, &peer_pub_key, endpoint, state.config.persistent_keepalive, peer_mtu, state.config.mtu == Some(0) || state.config.mesh_mode, &state.config.mode, 0, &state.config.psk, Some(state.client_sidecar.clone()), peer_mode, "UDP").await;
                                
                                // 服务端响应：只有当接收到的包不是响应包（或者处于 Mesh 模式主动握手阶段）才回发响应喵
                                // 这里简化为：始终回发响应，除非已经建立了连接喵
                                let established = get_established_peers(&state.config.interface).await;
                                if state.config.mesh_mode || !established.contains(&peer_pub_key) {
                                    let msg_base = state.pub_key.as_bytes().to_vec();
                                    let current_mtu = get_interface_mtu(&state.config.interface).unwrap_or(1420);
                                    let actual_tunnel_port = get_actual_listen_port(&state.config.interface).unwrap_or(0);
        ...
        async move {
            loop {
                ...
                for state in current_states {
                    ...
                    if state.pub_key.as_bytes() == &[0u8; 32] || state.config.native_wg_compat { continue; }
                                        ...
                                        if let Ok(ciphertext) = cipher.encrypt(Nonce::from_slice(&nonce_bytes), msg.as_slice()) {
                                            ...
                                            if tokio::io::AsyncWriteExt::write_all(&mut stream, &pkt).await.is_ok() {
                                                ...
                                                let mut ack_buf = [0u8; 128];
                                                                    ...
                                                                    Ok(Ok(n)) if n >= 12 + 32 => {
                                                                        ...
                                                                        if let Ok(decrypted) = cipher.decrypt(nonce, encrypted_part) {
                                                                            if decrypted.len() >= 36 {
                                                                                 ...
                                                                                 if let Some(mtu) = peer_mtu {
                                                                                     let mut lock = PEER_MTU_MAP.get_or_init(|| tokio::sync::Mutex::new(HashMap::new())).lock().await;
                                                                                     let peer_mtus = lock.entry(interface_inner.clone()).or_default();
                                                                                     peer_mtus.insert(peer_pub_key.clone(), mtu);
                                                                                     drop(lock);
                                                   ...
                                                   let _ = configure_peer(&interface_inner, &peer_pub_key, endpoint, None, peer_mtu, mesh_mode || state.config.mtu == Some(0), &mode_inner, 0, &psk_inner, Some(client_sidecar_clone), peer_mode, "Relay").await;
                                                                            }
                                                                        }
                                                                    },
                                                                    _ => {}
                                ...
                                for state in current_states {
                                    ...
                                    if let Ok(decrypted) = cipher.decrypt(nonce, encrypted_part) {
                                        if decrypted.len() >= 36 {
                                            ...
                                            let _ = configure_peer(&state.config.interface, &peer_pub_key, endpoint, state.config.persistent_keepalive, peer_mtu, state.config.mtu == Some(0) || state.config.mesh_mode, &state.config.mode, 0, &state.config.psk, Some(state.client_sidecar.clone()), peer_mode, "TCP-Server").await;
                                  ...
                                  for state in current_states {
                                     ...
                                     if let Ok(decrypted) = cipher.decrypt(nonce, encrypted) {
                                         ...
                                         if let Some(mtu) = peer_mtu {
                                             let mut lock = PEER_MTU_MAP.get_or_init(|| tokio::sync::Mutex::new(HashMap::new())).lock().await;
                                             let peer_mtus = lock.entry(state.config.interface.clone()).or_default();
                                             peer_mtus.insert(BASE64.encode(&decrypted[..32]), mtu);
                                             drop(lock);
                                         ...
                                         let _ = configure_peer(&state.config.interface, &peer_pub_key, ip_addr.to_string(), state.config.persistent_keepalive, peer_mtu, state.config.mtu == Some(0) || state.config.mesh_mode, "ip", 0, &state.config.psk, Some(state.client_sidecar.clone()), Some(&peer_mode.to_string()), "Raw IP").await;
...
async fn configure_peer(interface: &str, peer_pub_key: &str, mut endpoint: String, keepalive: Option<u16>, peer_mtu: Option<u16>, auto_sync_mtu: bool, mode: &str, _tcp_data_port: u16, _psk: &str, client_sidecar: Option<Arc<parking_lot::Mutex<Option<tokio::process::Child>>>>, peer_mode: Option<&String>, origin: &str) -> Result<()> {
    // 尝试解析域名端点喵
    ...
    let key = (interface.to_string(), peer_pub_key.to_string());
    {
        ...
        if let Some(old_ep) = cache.get(&key) {
            ...
            println!("喵！{} [{}] 识别到对端 {} 漫游变更: {} -> {} 喵！", interface, origin, peer_pub_key, old_ep, endpoint);
        } else {
            println!("喵！{} [{}] 首次识别到对端 {}: {} 喵！", interface, origin, peer_pub_key, endpoint);
        }
    }
    ...
    println!("喵！{} 对端 {} [模式: {}] 配置同步成功喵！", interface, peer_pub_key, actual_peer_mode);
